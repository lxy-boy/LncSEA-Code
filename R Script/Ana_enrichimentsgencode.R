...
enrichment<-function(x,y,pvalue=0.05,fdr,bonferroni,mix,max,path)
{	
	path=path
    input<-c(x)
    n<-length(input)
	...
	res<-c()
		...
		result<-matrix(0,length(data),12)
			...
			result[i,7]<-signif(phyper(length(inter)-1,n,13854-n,data[[i]]$length,lower.tail=FALSE),3)##pох
		...
		if(dim(result)[1]>=1)
		{
		result[,8]<-signif(p.adjust(result[,7],method="BH",n=length(result[,1]) ) ,3)###FDR
		result[,9]<-signif(p.adjust(result[,7],method="bonferroni",n=length(result[,1])),3)###Bonferroni
		}
	...
	write.table(res,paste0("accum/",path,T_type[j],".txt"),quote=F,sep="\t",col.names=T,row.names=F)
	print(T_type[j])
	...
	write.table(as.matrix(retp),paste0("accum/",path,"type.txt"),quote=F,sep="\t",col.names=F,row.names=F)
	}
...
type<-as.matrix(read.table(paste0("accum/",path,"database.txt")))

lnp<-read.table(paste0("accum/",path,"input.txt"))
if(dim(lnp)[1]==1)
{
...
setwd(".")
enrichment(ipt,type,pvalue,FDR,Bonferroni,min,max,path)
print(type)